...
//
// One-off script: copies every photo/audio/video referenced in Firestore
// (media, stories, people, trees) from the OLD Cloudinary account to your
// NEW one, then rewrites those Firestore documents to point at the new URLs.
//
// This does NOT touch your running app or Netlify — it's a standalone script
// you run once from your terminal.
//
// Setup:
//   1. Create a file named .env.migration in your project root (do NOT commit it)
//      containing:
//
//        FIREBASE_PROJECT_ID=...        (same values your old Netlify functions used)
//        FIREBASE_CLIENT_EMAIL=...
//        FIREBASE_PRIVATE_KEY="..."
//        NEW_CLOUDINARY_CLOUD_NAME=...   (from your NEW Cloudinary account's dashboard)
//        NEW_CLOUDINARY_API_KEY=...
//        NEW_CLOUDINARY_API_SECRET=...
//
//   2. Run a preview first (uploads to the new account so you can verify it
//      works, but does NOT write to Firestore yet):
//        node --env-file=.env.migration migrate-cloudinary.js
//
//   3. Once you've checked the new URLs look right, commit the change to Firestore:
//        node --env-file=.env.migration migrate-cloudinary.js --apply
//
// firebase-admin and cloudinary are already in your package.json, so no
// npm install should be needed. Requires Node 20.6+ for --env-file.
...
// Uploads oldUrl to the NEW account (Cloudinary fetches the remote URL itself —
// no need to download bytes manually). Skips if there's no url or it's already
// on the new account. Does not touch Firestore; the caller decides what to do
// with the returned new url/cloudinaryId.

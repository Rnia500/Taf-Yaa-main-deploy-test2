// src/components/VoiceStoryRecorder.jsx (v3 - clean, no branding)
...
const css = `
  ...
  .vsr-btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:10px 18px;border-radius:10px;font-size:13px;font-weight:600;border:none;cursor:pointer;transition:all .15s;font-family:inherit;}
  .vsr-btn:hover:not(:disabled){filter:brightness(.9);transform:scale(.98);}
  .vsr-btn:disabled{opacity:.5;cursor:not-allowed;}
  .vsr-select{padding:9px 32px 9px 10px;font-size:13px;border:1.5px solid #e5e7eb;border-radius:10px;background:#fff;color:#374151;cursor:pointer;outline:none;appearance:none;font-family:inherit;}
  .vsr-select:focus{border-color:#16a34a;}
  .vsr-textarea{width:100%;min-height:100px;padding:12px 14px;font-size:14px;border:1.5px solid #e5e7eb;border-radius:10px;font-family:inherit;line-height:1.7;color:#111827;resize:vertical;outline:none;transition:border-color .15s,box-shadow .15s;box-sizing:border-box;}
  .vsr-textarea:focus{border-color:#16a34a;box-shadow:0 0 0 3px rgba(22,163,74,.12);}
`;
...
function WaveBars({ active }) {
  return (
    <div style={{display:'flex',alignItems:'center',gap:3,height:28}}>
      {[0,1,2,3,4].map(i => (
        <div key={i} style={{width:4,borderRadius:4,background:active?'#16a34a':'#d1d5db',height:active?undefined:6,animation:active?`wave .8s ease ${i*.12}s infinite`:'none',transition:'background .3s'}}/>
      ))}
    </div>
  );
}
...
function Timer({ seconds }) {
  ...
  return <span style={{fontVariantNumeric:'tabular-nums',fontWeight:700,fontSize:22,color:'#111827'}}>{m}:{s}</span>;
}

function Progress({ label, percent }) {
  return (
    <div style={{animation:'fadeIn .3s ease',marginBottom:16}}>
...
const VoiceStoryRecorder = ({ treeId, personId, personName='this person', onStorySaved, onTranscript, compact=false }) => {
  ...
  useEffect(() => {
    if (isRecording) { timerRef.current = setInterval(() => setSeconds(s=>s+1), 1000); }
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      const result = await transcribeService.processAudio({
        audioBlob, userId, treeId, personId, language: lang,
  ...
  const reset = () => {
    ...
    setTranscript(''); setManualText(''); setError(null); setSavedStoryId(null);
    chunksRef.current = [];
  };
  ...
  return (
    <>
      ...
      <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:compact?14:18,overflow:'hidden',animation:'fadeIn .3s ease',boxShadow:'0 2px 12px rgba(0,0,0,0.06)'}}>

        {/* Header */}
        <div style={{background:'linear-gradient(135deg,#14532d,#166534)',padding:compact?'14px 18px':'20px 24px',display:'flex',alignItems:'center',gap:12}}>
          ...
          <div style={{flex:1}}>
            <h3 style={{margin:0,fontSize:compact?13:15,fontWeight:700,color:'#fff'}}>Voice Story Recorder</h3>
            <p style={{margin:'2px 0 0',fontSize:11,color:'rgba(255,255,255,0.65)'}}>Record a story for {personName} — we'll convert it to text</p>
          </div>
          {!isIdle && (
        ...
        <div style={{padding:compact?'14px 18px':'20px 24px'}}>
          ...
          {isIdle && (
            <div style={{marginBottom:18}}>
              <div style={{fontSize:11,fontWeight:600,color:'#6b7280',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:8}}>Language</div>
              ...
              {isManual && (
                <div style={{marginTop:10,background:'#f5f3ff',border:'1px solid #ddd6fe',borderRadius:10,padding:'10px 12px',fontSize:12,color:'#7c3aed',lineHeight:1.6}}>
                  ✍️ <strong>{selectedLang.name}</strong> is not yet supported by automatic voice recognition.
          ...
          {isManual && isIdle && (
            <div style={{animation:'fadeIn .3s ease'}}>
              <textarea className="vsr-textarea" placeholder={`Type the story in ${selectedLang.name}…`} value={manualText} onChange={e=>setManualText(e.target.value)} style={{marginBottom:12}}/>
              <button className="vsr-btn" onClick={handleManualSubmit} disabled={!manualText.trim()} style={{width:'100%',background:'linear-gradient(135deg,#14532d,#16a34a)',color:'#fff',fontSize:14}}>
          ...
          {!isManual && (isRecording||isPaused||isStopped) && (
            <div style={{background:isRecording?'#f0fdf4':'#f9fafb',border:`1.5px solid ${isRecording?'#bbf7d0':'#e5e7eb'}`,borderRadius:14,padding:'20px 16px',display:'flex',flexDirection:'column',alignItems:'center',gap:10,marginBottom:16,animation:'fadeIn .3s ease',transition:'all .3s'}}>
              {isRecording && (
              ...
              {isPaused && <div style={{width:46,height:46,borderRadius:'50%',background:'#f59e0b',display:'flex',alignItems:'center',justifyContent:'center'}}><Pause size={20} color="#fff"/></div>}
              {isStopped && <div style={{width:46,height:46,borderRadius:'50%',background:'#e5e7eb',display:'flex',alignItems:'center',justifyContent:'center'}}><Play size={20} color="#6b7280"/></div>}
          ...
          {/* Progress */}
          {isProcessing && <Progress label={progress.label||'Processing…'} percent={progress.percent}/>}

          {/* Result card */}
          {isDone && transcript && (
            <div style={{background:'#f0fdf4',border:'1.5px solid #86efac',borderRadius:14,overflow:'hidden',marginBottom:16,animation:'fadeIn .4s ease'}}>
              <div style={{background:'linear-gradient(135deg,#16a34a,#22c55e)',padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                ...
                <button onClick={handleCopy} style={{background:'rgba(255,255,255,0.2)',border:'none',borderRadius:7,cursor:'pointer',padding:'5px 10px',color:'#fff',fontSize:11,fontFamily:'inherit',display:'flex',alignItems:'center',gap:4}}>
                  {copied?'✓ Copied!':<><Copy size={12}/> Copy</>}
                </button>
              </div>
              <div style={{padding:'14px 16px'}}>
                <textarea className="vsr-textarea" value={transcript} onChange={e=>setTranscript(e.target.value)} style={{border:'1px solid #bbf7d0',background:'#fff',minHeight:80}}/>
                {savedStoryId && savedStoryId!=='manual' && (
                  <div style={{marginTop:8,display:'flex',alignItems:'center',gap:6,fontSize:12,color:'#16a34a'}}>
                    <BookOpen size={13}/>Story saved to {personName}'s profile!
                  </div>
                )}
              </div>
            </div>
          ...
          {(isStopped||isDone) && audioUrl && (
            <button className="vsr-btn" onClick={togglePlayback} style={{width:'100%',background:'#f3f4f6',color:'#374151',marginBottom:12}}>
              {isPlaying?<><Pause size={14}/> Pause</>:<><Play size={14}/> Play Recording</>}
            </button>
          )}
          ...
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            {isIdle && !isManual && (
              <button className="vsr-btn" onClick={startRecording} style={{flex:1,background:'linear-gradient(135deg,#14532d,#16a34a)',color:'#fff',fontSize:14,padding:'12px 20px'}}>
                <Mic size={16}/> Start Recording
              </button>
            )}
            {(isRecording||isPaused) && (
              <>
                <button className="vsr-btn" onClick={togglePause} style={{flex:1,background:isPaused?'#16a34a':'#f59e0b',color:'#fff'}}>
                  {isPaused?<><Play size={14}/> Resume</>:<><Pause size={14}/> Pause</>}
                </button>
                <button className="vsr-btn" onClick={stopRecording} style={{flex:1,background:'#dc2626',color:'#fff'}}>
                  <Square size={14}/> Stop
                </button>
              </>
            )}
            {isStopped && (
              <>
                <button className="vsr-btn" onClick={handleTranscribe} style={{flex:2,background:'linear-gradient(135deg,#14532d,#16a34a)',color:'#fff',fontWeight:700}}>
                  <Wand2 size={15}/> Convert to Text
                </button>
            ...
            {isDone && (
              <>
                ...
                {!savedStoryId && (
                  <button className="vsr-btn" onClick={()=>{onTranscript?.(transcript);reset();}} style={{flex:1,background:'linear-gradient(135deg,#14532d,#16a34a)',color:'#fff'}}>
          ...
          {isIdle && !isManual && (
            <p style={{margin:'12px 0 0',fontSize:11,color:'#9ca3af',textAlign:'center',lineHeight:1.7}}>
              🎙️ Speak clearly · Max 2 minutes · {selectedLang.flag} {selectedLang.name}
            </p>
          )}
        </div>
      </div>
